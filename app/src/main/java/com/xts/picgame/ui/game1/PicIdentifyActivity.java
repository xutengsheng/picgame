...
public class PicIdentifyActivity extends AppCompatActivity {
    public static final int TYPE_PIC_IDENTIFY = 0;//识别是什么,pic_identify
    public static final int TYPE_RECEPTIVE = 1;//练习物体名称,receptivelabeling
    ...
    private void showPic(int position) {
        Glide.with(this).load(mBeans.get(position).getUrl()).into(mIv);
        mStr = mRandomImageUtil.getTypeString(mBeans.get(position).getTypeid());
    ...
    private void addRandom() {
        DataBean random = mRandomImageUtil.random();
        mBeans.add(random);
    }
